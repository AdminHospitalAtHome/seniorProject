import {Alert, Pressable, Modal, Dimensions, StyleSheet, Text, ScrollView, View, TouchableOpacity} from 'react-native';
import * as React from 'react';
import { useState } from 'react';
import {TextInput, ListItem, Surface, Stack, Button } from "@react-native-material/core";
...
//global.dataSet = [];
const datas = [{"Date": "2022-10-13T19:31:00.000Z", "value": 88},
...
export default function PulseScreen() {
  const [isWSelected,setWSelection] = useState(false);
  const [Wchecked, setWChecked] = useState(false);
  const [isMSelected,setMSelection] = useState(false);
  const [Mchecked, setMChecked] = useState(false);
  const [isDSelected,setDSelection] = useState(false);
  const [Dchecked, setDChecked] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const values = datas.map((data) => data.value);
  const dates = datas.map((data) => Array.from(data.Date)[9] == 'T'?
